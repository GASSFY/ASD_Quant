...
# Usage: bash run_more_model.sh   (from repo root, or any cwd — script cds to its directory)
...
for MI in "${!MODEL_TYPES[@]}"; do
  ...
  SCALE_PATH="${MODEL_ROOT}/scale/asdq_w4.pt"
  MODEL_OUT="${MODEL_ROOT}"

  mkdir -p "$LOG_DIR" "${MODEL_ROOT}/scale"
